...
test("package command scans an input directory and writes an input manifest", () => {
  const dir = mkdtempSync(join(tmpdir(), "kuroko-package-folder-"));
  const caseDir = join(dir, "case_folder");
  const outDir = join(dir, "out");
  mkdirSync(caseDir);
  writeFileSync(
    join(caseDir, "request.docx"),
    createMinimalDocx([
      "外部パートナー向けに相談内容を整理してください。",
      "連絡先は taro@example.jp、案件 SG-2026-0001 です。",
    ]),
  );
  writeFileSync(
    join(caseDir, "notes.md"),
    "電話番号 03-1234-5678。希望日は2026年6月10日です。",
    "utf8",
  );
  writeFileSync(join(caseDir, "reference.pdf"), "%PDF-demo");
  writeFileSync(join(caseDir, "~$draft.docx"), "office temp");
  ...
  assert.equal(run.status, 0, run.stderr);
  assert.equal(
    existsSync(join(outDir, "handoff_package", "input_manifest.md")),
    true,
  );
  assert.equal(
    existsSync(join(outDir, "handoff_package", "input_manifest.json")),
    true,
  );
  assert.equal(
    existsSync(join(outDir, "handoff_package", "sanitized", "request.docx.md")),
    true,
  );

  const manifest = JSON.parse(
    readFileSync(join(outDir, "handoff_package", "input_manifest.json"), "utf8"),
  ) as {
    entries: Array<{ relativePath: string; status: string; reason: string }>;
  };
  assert.deepEqual(
  ...
  const sanitizedDocx = readFileSync(
    join(outDir, "handoff_package", "sanitized", "request.docx.md"),
    "utf8",
  );
  assert.match(sanitizedDocx, /\{\{EMAIL_001\}\}/);
  assert.match(sanitizedDocx, /\{\{CASE_ID_001\}\}/);
  assert.doesNotMatch(sanitizedDocx, /taro@example\.jp/);
  assert.doesNotMatch(sanitizedDocx, /SG-2026-0001/);

  const audit = readFileSync(join(outDir, "handoff_package", "audit_log.json"), "utf8");
  assert.match(audit, /input_manifest\.json/);
  assert.doesNotMatch(audit, /taro@example\.jp/);
});
